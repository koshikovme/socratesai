...
1. `ml/policy_dataset.csv`
   ...
   - Purpose: weak-supervision / rule-policy imitation baseline.
   - Meaning: the model learns to reproduce the current policy behavior from exported interaction-style features.

2. `experiments/results/problem-suite-1080/rule/events.csv`
...
3. `ml/problem_suite_policy_dataset.csv`
   - Rows: 1,080
...
The study also runs ablations without history features, analyzer features, suspicious-region features, and last-feedback-action features. XGBoost and LightGBM are reported as optional skipped models unless those packages are installed.
...
```powershell
python .\ml\train_policy_model.py `
  --input .\ml\policy_dataset.csv `
  --output-prefix .\ml\mentor_policy_model `
...
| Train rows | 7,353 |
| Test rows | 1,777 |
| Split | student-level group holdout |
| Accuracy | 1.0000 |
| Macro F1 | 1.0000 |
